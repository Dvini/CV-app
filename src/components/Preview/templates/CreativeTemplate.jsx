...
export const CreativeTemplate = React.memo(function CreativeTemplate() {
  const { data, sectionOrder, sectionColumns, getMarginStyle, getPhotoStyle, creativeHeaderBg } = useCV();
  ...
  return (
    <div className="cv-page template-creative">
        ...
        <div className="cv-header-creative-inner" style={{ padding: `${getMarginStyle('container', true).paddingTop} ${getMarginStyle('container', true).paddingRight} 1rem ${getMarginStyle('container', true).paddingLeft}` }}>
          <div className="cv-header-creative-row">
            {data.personal.showPhoto && data.personal.photo && (
              <img src={data.personal.photo} alt={`Zdjęcie profilowe — ${data.personal.fullName || ''}`} className="cv-header-photo cv-header-photo--creative" style={{ ...getPhotoStyle(), border: '3px solid rgba(255,255,255,0.5)' }} />
            )}
      ...
      <div className="cv-creative-body">
        <div className="cv-creative-side" style={{ paddingLeft: getMarginStyle('container', showClauseFooter).paddingLeft, paddingTop: '1rem', paddingBottom: showClauseFooter ? '0' : getMarginStyle('container', false).paddingBottom, paddingRight: '0.75rem' }}>
          {sideSections.map((section) => renderCVSection(section, 'side'))}
        </div>
        <div className="cv-creative-main" style={{ paddingRight: getMarginStyle('container', showClauseFooter).paddingRight, paddingTop: '1rem', paddingBottom: showClauseFooter ? '0' : getMarginStyle('container', false).paddingBottom, paddingLeft: '0.75rem' }}>
